...
class TokenManager {
  constructor(accountConfig) {
    ...
    this.auth = new CognitoAuth(
      accountConfig.cognito.username,
      accountConfig.cognito.password
    );
    this.tokenPath = path.join(
      __dirname,
      `tokens_${accountConfig.accountIndex}.json`
    );
  }
...
async function getTokens(tokenPath) {
  try {
    if (!fs.existsSync(tokenPath))
      throw new Error(`Tokens file not found at ${tokenPath}`);
...
if (!isMainThread) {
  ...
  let previousStats = userConfigs.map(() => ({
    validCount: 0,
    invalidCount: 0,
  ...
  async function runValidationProcess(tokenManager, accountIndex) {
    try {
      ...
      if (
        previousStats[accountIndex].validCount === 0 &&
        previousStats[accountIndex].invalidCount === 0
      ) {
          ...
          workers.push(
            new Promise((resolve) => {
              ...
              worker.on("error", (error) =>
                resolve({ success: false, error: error.message, proxy })
              );
              worker.on("exit", () =>
                resolve({ success: false, error: "Worker exited", proxy })
              );
            })
          );
        });
      ...
      const actualValidIncrease =
        newValidCount - previousStats[accountIndex].validCount;
      const actualInvalidIncrease =
        newInvalidCount - previousStats[accountIndex].invalidCount;
      previousStats[accountIndex].validCount = newValidCount;
      previousStats[accountIndex].invalidCount = newInvalidCount;
      updateStats(
        updatedUserData,
        accountIndex,
        proxies.length > 0 ? proxies[accountIndex % proxies.length] : null
      );
    } catch (error) {
  ...
  function updateStats(userData, accountIndex, proxy) {
    ...
    previousStats[accountIndex].validCount =
      userData.stats.stork_signed_prices_valid_count || 0;
    previousStats[accountIndex].invalidCount =
      userData.stats.stork_signed_prices_invalid_count || 0;
  ...
  function displayAllStats() {
    ...
    const errorCount = previousStats.filter((stat) =>
      stat.status.startsWith("Error")
    ).length;
    ...
    const rows = userConfigs.map((config, index) => {
      ...
      const percentage =
        total > 0 ? ((stats.validCount / total) * 100).toFixed(2) : "0.00";
      return {
